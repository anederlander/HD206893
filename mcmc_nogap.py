...
def lnprob(p1):
    r_in, delta_r, log_m_disk, f_star, position_angle, inclination, xoffs, yoffs, pp = p1
    priors_r_in = [0, 10000]
    ...
    total_model(x,imres=0.05,distance=40.8,nchans=5,freq0=222.516,xnpix=256,vsys=-12.4510196,PA=position_angle,offs=[xoffs,yoffs],modfile=model_name,isgas=False, x_offset=xoffs, y_offset=yoffs, stellar_flux=f_star)
...
def emcee(nsteps=400, ndim=9, nwalkers=18, walker_1=50.0, walker_2=30.0, walker_3=-6, walker_4=1.65e-5, walker_5=60, walker_6=40, walker_7=.2, walker_8=.8, walker_9=1.0 ,sigma_1=50.0, sigma_2=50.0, sigma_3=2, sigma_4=8e-6, sigma_5=20, sigma_6=10, sigma_7=.5, sigma_8=.5, sigma_9=1.0, restart=True):
    ...
    if restart == False:
        p0 = np.random.normal(loc=(walker_1, walker_2, walker_3, walker_4, walker_5, walker_6, walker_7, walker_8, walker_9), size=(nwalkers, ndim), scale=(sigma_1, sigma_2, sigma_3, sigma_4, sigma_5, sigma_6, sigma_7, sigma_8, sigma_9))
    else:
        #read from csv file
        dg = pd.read_csv("/Volumes/disks/ava/hd206893/disk_model/disk_model3_new/chain_1000steps_june9_nogap.csv")
        p0 = np.zeros([nwalkers,ndim])
        for i in range(nwalkers):
            p0[i,0] = dg['r_in'].iloc[-(nwalkers-i+1)]
            p0[i,1] = dg['delta_r'].iloc[-(nwalkers-i+1)]  #future versions should be delta_r
            ...
            p0[i,8] = dg['pp'].iloc[-(nwalkers-i+1)]
    import emcee
    ...
    for i, result in enumerate(run):
        ...
        df.columns = ['r_in', 'delta_r', 'm_disk', 'f_star', 'position_angle', 'inclination', 'xoffs', 'yoffs', 'pp', 'lnprob']
        df.to_csv('chain_400steps_june11_nogap.csv', mode='w')
    ...
    df.to_csv('chain_500steps_final8params.csv', mode='a')'''
    ...
    fig, (ax0,ax1,ax2, ax3, ax4, ax5, ax6, ax7, ax8, ax9) = plt.subplots(ndim+1)
    ...
    for i in range(0, nwalkers):
        ...
        ax8.plot(x, df['pp'][i::nwalkers])
        ax9.plot(x, df['lnprob'][i::nwalkers])
    fig.suptitle('r_in, delta_r, m_disk, f_star, position_angle, inclination, xoffs, yoffs, pp, lnprob')
    plt.show(block=False)
