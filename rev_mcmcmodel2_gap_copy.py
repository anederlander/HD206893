...
def lnprob(p1):
    r_in, delta_r, log_m_disk, f_star, position_angle, inclination, xoffs_stellar, yoffs_stellar, r_in_gap, delta_r_gap, pp = p1
    priors_r_in = [0, 10000]
    ...
    priors_pp = [-5, 5]
    #priors_xoffs_disk = [-5, 5] #disk
    #priors_yoffs_disk = [-5, 5] #disk
    ...
    x = Disk(params=[-0.5,m_disk,pp,r_in,r_out,150.,inclination,2.3,1e-4,0.01,33.9,[.79,1000],[10.,1000], -1, 500, 500, 2.83, 0.1])
...
def emcee(nsteps=400, ndim=11, nwalkers=26, walker_1=10.0, walker_2=150.0, walker_3=-6, walker_4=1.65e-5, walker_5=60, walker_6=40, walker_7=.2, walker_8=.8, walker_9=40, walker_10=40, walker_11=1.0, sigma_1=30.0, sigma_2=30.0, sigma_3=2, sigma_4=8e-6, sigma_5=20, sigma_6=10, sigma_7=0.5, sigma_8=0.5, sigma_9=10, sigma_10=10, sigma_11=1.0, restart=True):
    ...
    :param walker_9:            the first parameter for the 9th dimension - r_in for gap
    :param walker_10:           the first parameter for the 10th dimension - delta_r for gap
    ...
    if restart == False:
        p0 = np.random.normal(loc=(walker_1, walker_2, walker_3, walker_4, walker_5, walker_6, walker_7, walker_8, walker_9, walker_10, walker_11), size=(nwalkers, ndim), scale=(sigma_1, sigma_2, sigma_3, sigma_4, sigma_5, sigma_6, sigma_7, sigma_8, sigma_9, sigma_10, sigma_11))
    else:
        #read from csv file
        dg = pd.read_csv("/Volumes/disks/ava/hd206893/disk_model/disk_model3_new/chain_1000steps_june9_gap.csv")
        ...
        for i in range(nwalkers):
            ...
            p0[i,8] = dg['r_in_gap'].iloc[-(nwalkers-i+1)]
            p0[i,9] = dg['delta_r_gap'].iloc[-(nwalkers-i+1)]
            p0[i,10] = dg['pp'].iloc[-(nwalkers-i+1)]
    ...
    for i, result in enumerate(run):
        ...
        df.columns = ['r_in', 'delta_r', 'm_disk', 'f_star', 'position_angle', 'inclination', 'xoffs_stellar', 'yoffs_stellar', 'r_in_gap', 'delta_r_gap', 'pp', 'lnprob']
        df.to_csv('chain_400steps_june11_gap.csv', mode='w')
    ...
    for i in range(nwalkers):
        ...
        fig, (ax0,ax1,ax2, ax3, ax4, ax5, ax6, ax7, ax8, ax9, ax10, ax11) = plt.subplots(ndim+1)
    ...
    for i in range(0, nwalkers):
        ...
        ax8.plot(x, df['r_in_gap'][i::nwalkers])
        ax9.plot(x, df['delta_r_gap'][i::nwalkers])
        ax10.plot(x, df['pp'][i::nwalkers])
        ax11.plot(x, df['lnprob'][i::nwalkers])
        fig.suptitle('r_in, delta_r, m_disk, f_star, position_angle, inclination, xoffs_stellar, yoffs_stellar, r_in_gap, delta_r_gap, pp, lnprob')
        plt.show(block=False)
