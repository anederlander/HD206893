...
'''
    ...
    # Here is a good place to add a star in the appropriate pixel - this is currently Meredith's pseudocode and won't run
    #xpix = np.rint(cen+x_offset/xpixscale)
    #ypix = np.rint(cen+y_offset/xpixscale)
    #im_s[xpix,ypix,:]+=star_flux

    #Ava's Code:

    #cen = np.array([xnpix/2.+.5, xnpix/2.+.5])   # - central pixel location
    #print xpixscale
    xpix = int(np.rint((xnpix/2.+.5) - x_offset/xpixscale))
    ypix = int(np.rint((xnpix/2.+.5) + y_offset/xpixscale))
    #print(xpix, ypix, stellar_flux)
    #print(np.shape(im_s[xpix,ypix,:]))

    im_s[xpix,ypix,:]+=np.repeat(stellar_flux,nchans)
    
    ###### Gaussian Circle ######
    D = 1.2e4 #diameter of telescope = 12m =  1.2e4 mm
    lambda1 = 1.33 #wavelength of observation = 1.33mm
    FWHM = ((1.22*lambda1/D) * 180/math.pi * 3600) / imres # arcsecs 

    peak = 1 # max value of 2D Gaussian in Jy/pix
    FWHMx = FWHM 
    FWHMy = FWHM
    PA = 16 #rotation of the Gaussian


    x = np.arange(0, xnpix, 1, float)
    y = x[:,np.newaxis]
    ...
    a = np.cos(PA)**2 / (2 * sigmax**2) + np.sin(PA)**2 / (2 * sigmay**2)
    b = -np.sin(2*PA) / (4 * sigmax**2) + np.sin(2*PA) / (4 * sigmay**2)
    b = -b
    c = np.sin(PA)**2 / (2 * sigmax**2) + np.cos(PA)**2 / (2 * sigmay**2)

    x0 = y0 = xnpix // 2

    #return peak * np.exp(-(a*(x-x0)**2 + 2*b*(x-x0)*(y-y0) + c*(y-y0)**2))

    pb=peak * np.exp(-(a*(x-x0)**2 + 2*b*(x-x0)*(y-y0) + c*(y-y0)**2))
    im_s*=pb    
